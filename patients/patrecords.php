...
<?php 
	//include("menu.php"); 
	mysqli_query($con,"DELETE FROM patients WHERE email = '$_GET[paid]'");
	if(isset($_POST["patidbtn"]))
	{
		$resapp = mysqli_query($con,"SELECT * FROM patients where id='$_POST[textfield]'");
		$_POST[textfield2] ="";
	}
	else if(isset($_POST["patcontbtn"]))
	{
		$resapp = mysqli_query($con,"SELECT * FROM patients where contactno='$_POST[textfield2]'");
			$_POST[textfield]="";
	}
	else
	{
...
<div id="container">
          ...
          while($row1 = mysql_fetch_array($resapp))
  {
	  ...
	  ?> 	 
        <tr>
          <td height="40"><?php echo $row1["id"]; ?></td>
          <td><?php echo $row1["fullName"]; ?></td>
          
          <td><?php echo $row1["gender"]; ?></td>
          <td><?php echo $row1["email"]; ?></td>
          <td>&nbsp;<?php echo $row1["contactno"]; ?></td>
          <td><?php echo $row1["address"]. "". $row1["city"]; ?></td>
